...
function impactRing(scene: SceneLike, x: number, y: number, tint: number, finishing: boolean) {
  const ring = scene.add.circle(x, y, finishing ? 18 : 13, tint, 0.06).setStrokeStyle(finishing ? 4 : 3, tint, 0.9).setDepth(980).setBlendMode(Phaser.BlendModes.ADD);
...
function directionalSparks(scene: SceneLike, x: number, y: number, tint: number, sourceX: number, sourceY: number, count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const spark = scene.add.sprite(x, y, "spark").setTint(i % 3 === 0 ? 0xfff4e7 : tint).setDepth(981).setScale(Phaser.Math.FloatBetween(0.32, 0.7)).setAlpha(0.95);
    scene.tweens.add({ targets: spark, x: x + Math.cos(a) * d, y: y + Math.sin(a) * d, alpha: 0, scale: 0.12, duration: Phaser.Math.Between(115, 190), ease: "Quad.easeOut", onComplete: () => spark.destroy() });
  }
}
...
function bossFlashEcho(scene: SceneLike, boss: Phaser.Physics.Arcade.Sprite, x: number, y: number, finishing: boolean) {
  ...
  if (!texture) return;
  const echo = scene.add.sprite(x, y, texture, boss.frame?.name).setDepth((boss.depth ?? 20) + 0.45).setFlipX(boss.flipX).setAngle(boss.angle ?? 0).setScale(boss.scaleX, boss.scaleY).setTint(0xffffff).setAlpha(finishing ? 0.88 : 0.72).setBlendMode(Phaser.BlendModes.ADD);
  const playerX = scene.player?.x ?? x - 1, playerY = scene.player?.y ?? y;
  const dx = x - playerX, dy = y - playerY, len = Math.max(1, Math.hypot(dx, dy)), kick = finishing ? 5 : 3;
  scene.tweens.add({ targets: echo, x: x + (dx / len) * kick, y: y + (dy / len) * kick * 0.65, scaleX: echo.scaleX * (finishing ? 1.07 : 1.04), scaleY: echo.scaleY * (finishing ? 0.92 : 0.95), alpha: 0, duration: finishing ? 145 : 105, ease: "Quad.easeOut", onComplete: () => echo.destroy() });
}

function isWeariness(scene: SceneLike, bossName = String(scene.bossName ?? "")) { return scene.save?.current_zone === ACT4_ZONE && bossName === WEARINESS_NAME; }
function isAct4ShardGuardian(scene: SceneLike, bossName: string) { return scene.save?.current_zone === ACT4_ZONE && /shard guardian/i.test(bossName); }
...
function bossImpact(scene: SceneLike, boss: Phaser.Physics.Arcade.Sprite | null, x: number, y: number, finishing: boolean, mariaHit: boolean) {
  const now = Number(scene.time?.now ?? 0), last = Number(scene.__combatBossReactionAt ?? -Infinity);
...
function tuneShardGuardianStats(scene: SceneLike) {
  ...
  if (!boss.getData?.("act4-guardian-hp-3000")) {
    scene.bossHp = 3000; scene.bossMax = 3000;
...
function removeWearinessPulse(scene: SceneLike) {
  const halo = scene.bossHalo;
  if (halo) { try { scene.tweens?.killTweensOf?.(halo); halo.setAlpha?.(0); halo.setVisible?.(false); } catch {} }
  try { scene.bolts?.clear?.(true, true); } catch {}
  if (scene.bossShotTimer) { try { scene.bossShotTimer.remove?.(); } catch {} scene.bossShotTimer = undefined; }
}
...
function tuneWeariness(scene: SceneLike) {
  ...
  boss.setAlpha(1); removeWearinessPulse(scene);
  ...
  if (scene.bossTimer && scene.__wearinessMobTimer !== scene.bossTimer) { scene.__wearinessMobTimer = scene.bossTimer; scene.bossTimer.timeScale = 2; }
}
...
function swingAccent(scene: SceneLike) {
  ...
  const x = player.x + Math.cos(dir) * w.reach * 0.72, y = player.y + Math.sin(dir) * w.reach * 0.72;
  const streak = scene.add.arc(player.x, player.y, Math.max(18, w.reach * 0.82), Phaser.Math.RadToDeg(dir) - 34, Phaser.Math.RadToDeg(dir) + 34, false, w.color, 0.18).setStrokeStyle(2, w.color, 0.72).setDepth(20).setBlendMode(Phaser.BlendModes.ADD);
...
function halveAct4SwordKnockback(scene: SceneLike) {
  ...
  const weaponId = String(scene.save?.equipped_weapon ?? "").toLowerCase(), weapon = scene.equippedWeapon?.();
  if (!weapon || !weaponId.includes("sword") || !scene.player?.active) return;
  const reach = Math.max(28, Number(weapon.reach ?? 44) * 1.35), px = scene.player.x, py = scene.player.y;
...
export function installCombatImpactPolish(QuestScene: SceneCtor) {
  ...
  proto.attack = function combatImpactAttack(this: SceneLike, ...args: any[]) {
    ...
    if (canSwing) { this.__combatImpactMariaSwingUntil = Number(this.time?.now ?? 0) + MARIA_CONTACT_WINDOW; swingAccent(this); }
  ...
  if (typeof originalUpdate === "function") proto.update = function(this: SceneLike, time: number, delta: number) {
    ...
    if (this.save?.current_zone === ACT4_ZONE) {
      ...
      } else if (name === WEARINESS_NAME) tuneWeariness(this);
    }
    return result;
  };
  ...
  proto.damageBoss = function(this: SceneLike, amount: number, ...args: any[]) {
    ...
    const boss = this.boss as Phaser.Physics.Arcade.Sprite | null, hpBefore = Number(this.bossHp ?? 0), x = boss?.x ?? 0, y = boss?.y ?? 0;
    ...
    if (boss && hpAfter < hpBefore) {
      ...
      try { bossImpact(this, boss?.active && boss.scene ? boss : null, x, y, hpAfter <= 0, mariaHit); } catch (err) { console.warn?.("[quest] combat impact skipped after boss state changed", err); }
    }
    return result;
  };
