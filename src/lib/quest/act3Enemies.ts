...
function makeTextures(scene: SceneLike) {
  ...
  canvas(scene, WISP_KEY, 64, 72, (c) => {
    // Painted-in aura: no follower timers or extra moving objects.
    ...
    // Shadow under the floating creature.
    c.fillStyle = "rgba(10,12,22,.36)";
    c.beginPath(); c.ellipse(32, 66, 18, 4, 0, 0, Math.PI * 2); c.fill();

    // Curling black-purple smoke silhouette.
    c.strokeStyle = "#1b1428"; c.lineWidth = 11; c.lineCap = "round";
    c.beginPath();
    c.moveTo(31, 43); c.bezierCurveTo(18, 50, 21, 60, 33, 61); c.bezierCurveTo(43, 62, 44, 68, 35, 70);
    c.stroke();
    c.strokeStyle = "#452758"; c.lineWidth = 5;
    c.beginPath(); c.moveTo(28, 43); c.bezierCurveTo(19, 51, 25, 57, 35, 59); c.stroke();

    // Heavy dark heart rim gives it a clear silhouette at gameplay scale.
    heartPath(c, 32, 29, 18);
    c.fillStyle = "#171222"; c.fill();
    c.strokeStyle = "#6f397e"; c.lineWidth = 3; c.stroke();

    heartPath(c, 32, 29, 13);
    const core = c.createLinearGradient(22, 18, 42, 42);
    core.addColorStop(0, "#ffb1d0"); core.addColorStop(0.45, "#ff5f9f"); core.addColorStop(1, "#b72f78");
    c.fillStyle = core; c.fill();

    // Bright central crack — visually explains the "doubt" identity.
  ...
  canvas(scene, VEILED_KEY, 76, 92, (c) => {
    ...
    glow.addColorStop(0, "rgba(255,125,173,.28)");
    glow.addColorStop(0.58, "rgba(240,200,108,.09)");
    glow.addColorStop(1, "rgba(20,16,30,0)");
    c.fillStyle = glow; c.fillRect(2, 4, 72, 72);

    c.fillStyle = "rgba(12,14,24,.38)";
    c.beginPath(); c.ellipse(38, 86, 24, 5, 0, 0, Math.PI * 2); c.fill();

    // Long black-purple gown with a strong triangular silhouette.
    c.fillStyle = "#171522";
    c.beginPath(); c.moveTo(31, 36); c.lineTo(45, 36); c.lineTo(58, 82); c.lineTo(18, 82); c.closePath(); c.fill();
    c.fillStyle = "#33243f";
    c.beginPath(); c.moveTo(34, 39); c.lineTo(42, 39); c.lineTo(49, 77); c.lineTo(27, 77); c.closePath(); c.fill();
    c.fillStyle = "#522f5f";
    c.beginPath(); c.moveTo(30, 48); c.lineTo(37, 43); c.lineTo(37, 77); c.lineTo(26, 77); c.closePath(); c.fill();

    // Gold wedding trim visible even when the sprite is moving.
    c.strokeStyle = "#e9be63"; c.lineWidth = 3;
    c.beginPath(); c.moveTo(26, 68); c.lineTo(50, 68); c.moveTo(31, 43); c.lineTo(38, 49); c.lineTo(45, 43); c.stroke();
    c.fillStyle = "#f4d37d"; c.fillRect(36, 69, 4, 8);

    // Cream bridal veil: wide, asymmetrical and unmistakable.
    c.fillStyle = "#efe6d8";
    c.beginPath(); c.moveTo(19, 13); c.quadraticCurveTo(38, -1, 56, 13); c.lineTo(61, 49); c.lineTo(50, 42); c.lineTo(46, 56); c.lineTo(38, 43); c.lineTo(29, 55); c.lineTo(25, 41); c.lineTo(15, 49); c.closePath(); c.fill();
    c.fillStyle = "#c9bba7";
    c.beginPath(); c.moveTo(20, 15); c.quadraticCurveTo(38, 3, 55, 15); c.lineTo(53, 21); c.quadraticCurveTo(38, 12, 22, 21); c.closePath(); c.fill();
    c.strokeStyle = "#d6a957"; c.lineWidth = 2;
    c.beginPath(); c.moveTo(21, 15); c.quadraticCurveTo(38, 4, 55, 15); c.stroke();

    // Empty face framed by veil, with glowing heart-mask at the center.
    c.fillStyle = "#0d0c16";
    c.beginPath(); c.ellipse(38, 28, 12, 13, 0, 0, Math.PI * 2); c.fill();
    heartPath(c, 38, 29, 7);
    c.fillStyle = "#ff639f"; c.fill();
    c.strokeStyle = "#ffd7e6"; c.lineWidth = 1.5; c.stroke();
    c.fillStyle = "#fff6fa"; c.fillRect(35, 25, 2, 2); c.fillRect(40, 25, 2, 2);

    // Arms separate clearly from the gown.
    c.strokeStyle = "#24202d"; c.lineWidth = 7; c.lineCap = "round";
    c.beginPath(); c.moveTo(27, 45); c.lineTo(15, 58); c.moveTo(49, 45); c.lineTo(59, 59); c.stroke();
    c.strokeStyle = "#d1a55c"; c.lineWidth = 2;
    c.beginPath(); c.moveTo(26, 45); c.lineTo(15, 58); c.moveTo(50, 45); c.lineTo(58, 58); c.stroke();

    // Rose lantern in the right hand.
    c.strokeStyle = "#d6a957"; c.lineWidth = 2;
    c.beginPath(); c.moveTo(59, 58); c.lineTo(64, 63); c.stroke();
    c.fillStyle = "#3a243a"; c.fillRect(59, 62, 11, 15);
    c.strokeStyle = "#e9be63"; c.strokeRect(59, 62, 11, 15);
    c.fillStyle = "#ff729f"; c.fillRect(62, 66, 5, 7);
    c.fillStyle = "#ffd9e5"; c.fillRect(63, 67, 2, 2);

    // A few petals keep it tied to Haven rather than reading as generic undead.
...
function havenBurst(scene: SceneLike, enemy: any, heavy = false) {
  ...
  for (let i = 0; i < count; i++) {
    const mote = scene.add.rectangle(
      ...
      enemy.y + Phaser.Math.Between(-9, 7),
