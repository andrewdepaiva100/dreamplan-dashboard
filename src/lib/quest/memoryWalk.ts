...
class MemoryWalkScene extends Phaser.Scene {
  ...
  private buildMemoryWalk() {
    ...
    // Layered star field: deterministic positions keep the scene stable on replay.
    for (let i = 0; i < 115; i++) {
      ...
      if (i % 13 === 0) {
        this.tweens.add({ targets: star, alpha: { from: 0.25, to: 0.9 }, scale: { from: 0.7, to: 1.35 }, duration: 1500 + (i % 5) * 220, yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
      }
    }

    // A luminous path through the memories. It has no collision and cannot block Maria.
    ...
    this.add.text(86, 325, "THE MEMORY WALK", {
      fontFamily: "Georgia, serif", fontSize: "24px", color: "#f8e7b1", letterSpacing: 5,
      stroke: "#0b1028", strokeThickness: 5,
    }).setOrigin(0, 0.5).setDepth(20).setAlpha(0.88);
    this.add.text(88, 354, "Walk forward. Let every chapter pass beside you.", {
      fontFamily: "Georgia, serif", fontSize: "13px", fontStyle: "italic", color: "#d8def5",
    ...
    this.time.delayedCall(650, () => {
      ...
      this.parentScene?.emitToast?.("The stars hold their breath. Maria walks through what brought her here.");
    });
  }
  ...
  private addMemory(x: number, act: string, title: string, tint: number, landmarkKey: string, accents: string[], line: string) {
    ...
    const maxW = 180;
    if (landmark.width > 0) landmark.setScale(Math.min(1.25, maxW / landmark.width));

    // Two offset echoes make the memory feel painted in light without shaders.
    for (const ox of [-12, 12]) {
      this.add.sprite(x + ox, 354, landmarkKey).setDepth(5).setTint(tint).setAlpha(0.07).setScale(landmark.scaleX * 1.04, landmark.scaleY * 1.04);
    }
  ...
  update(_time: number, delta: number) {
    ...
    // The walk is intentionally forward-focused, but Maria can step back to revisit a memory.
    const speed = 112;
    this.maria.x = Phaser.Math.Clamp(this.maria.x + Phaser.Math.Clamp(dx, -1, 1) * speed * Math.min(delta, 50) / 1000, 72, 2365);
  ...
  private finishToCathedral() {
    ...
    try {
      ...
      console.warn?.("[quest] Memory Walk restart fallback", err);
    }

    // Last-resort recovery: never leave the player stranded in the bridge scene.
    try {
      this.scene.stop();
...
function installMemoryWalk() {
  ...
  proto.advanceZone = function memoryWalkAdvanceZone(this: any, ...args: any[]) {
    if (this.save?.current_zone !== ACT4 || this.__memoryWalkLaunching) {
    ...
    // Preserve the portal's existing flash, then dissolve Act IV into the memory space.
    try {
      this.cameras.main.fadeOut(700, 5, 8, 23);
      this.time.delayedCall(740, launch);
    } catch {
      launch();
    }
  };
