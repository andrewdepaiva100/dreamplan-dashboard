...
function ensureKeeperTextures(scene: any) {
  // Hand-pixeled at the same tiny native scale as the named overworld cast.
  if (!scene.textures.exists("evelyn-keeper")) {
    ...
    // sage coat silhouette and dark outline
  ...
  if (!scene.textures.exists("keeper-nook")) {
    ...
    // rose trellis: irregular timber with vine clusters
    ...
    // substantial potting bench, plank-by-plank rather than flat geometry
    ...
    // terracotta pots with rims/highlights
    ...
    // seed packets + folded cloth + shears
...
function addKeeperStation(scene: any) {
  ...
  // The nook sits left/up from Evelyn, leaving a clean approach lane for Maria.
  ...
  // Purposeful stepping-stone entrance, kept behind the character and props.
  const stones:any[] = [];
...
function refreshKeeperGarden(scene: any, force = false) {
  ...
  // Progress grows outward around the nook instead of piling props under Evelyn.
  if (n >= 1) {
    for (const [dx,dy] of [[-138,52],[-116,66],[-82,73],[-54,70]]) flower(dx,dy,0xb9f28f,.5);
  }
  if (n >= 2) {
  ...
  if (n >= 4) {
    // Full-bloom reveal stays to Evelyn's right, leaving her and Maria unobstructed.
...
function showEvelynDialogue(scene:any,line:string) {
  ...
  overlay.innerHTML=`<div style="position:relative;width:min(860px,96vw);display:flex;align-items:flex-end;gap:14px"><button data-close aria-label="Skip dialogue" style="position:absolute;right:2px;top:-42px;z-index:2;width:36px;height:36px;border-radius:999px;border:1px solid rgba(240,210,125,.75);background:rgba(10,16,34,.96);color:#efd477;font-weight:800;cursor:pointer">✕</button><img src="${evelynPortrait}" alt="Evelyn" style="width:min(240px,28vw);aspect-ratio:1;object-fit:cover;border-radius:18px;border:2px solid #d7b65e;box-shadow:0 18px 50px rgba(0,0,0,.48),0 0 30px rgba(215,182,94,.18)"/><button data-advance style="position:relative;flex:1;min-height:190px;text-align:left;overflow:hidden;border-radius:20px;border:2px solid rgba(215,182,94,.82);background:rgba(10,16,34,.96);box-shadow:0 20px 55px rgba(0,0,0,.48);padding:22px 24px;cursor:pointer;color:white"><div style="position:absolute;inset:0;background:linear-gradient(135deg,rgba(215,182,94,.12),transparent 48%,rgba(145,170,120,.11));pointer-events:none"></div><div style="position:relative"><div style="display:flex;gap:12px;align-items:baseline;flex-wrap:wrap"><strong style="font-family:Georgia,serif;font-size:23px;color:#efd477">Evelyn</strong><span style="font-size:10px;color:rgba(255,255,255,.58);letter-spacing:.18em;text-transform:uppercase">Keeper of the Four Seasons</span></div><p data-text style="min-height:78px;margin:13px 0 0;font-family:Georgia,serif;font-style:italic;font-size:16px;line-height:1.65;color:rgba(255,255,255,.96)"></p><div style="display:flex;justify-content:space-between;align-items:center;margin-top:16px"><span style="display:flex;gap:6px"><i style="width:20px;height:4px;border-radius:9px;background:#d7b65e"></i><i style="width:20px;height:4px;border-radius:9px;background:rgba(255,255,255,.18)"></i><i style="width:20px;height:4px;border-radius:9px;background:rgba(255,255,255,.18)"></i></span><span data-hint style="font-size:10px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#efd477">…</span></div></div></button></div>`;
  if(window.matchMedia("(max-width: 640px)").matches){const wrap=overlay.firstElementChild as HTMLElement;wrap.style.alignItems="stretch";const img=overlay.querySelector("img") as HTMLElement;img.style.width="78px";img.style.height="78px";img.style.position="absolute";img.style.left="16px";img.style.top="16px";img.style.zIndex="3";const panel=overlay.querySelector("[data-advance]") as HTMLElement;panel.style.padding="20px 18px 18px 110px";panel.style.minHeight="210px";}
  document.body.appendChild(overlay);
  const text=overlay.querySelector("[data-text]") as HTMLElement;const hint=overlay.querySelector("[data-hint]") as HTMLElement;let i=0;
  let timer=window.setInterval(()=>{i+=2;text.textContent=`“${line.slice(0,i)}${i>=line.length?"”":""}`;if(i>=line.length){window.clearInterval(timer);hint.textContent="Tap to close";}},18);
  const close=()=>{window.clearInterval(timer);overlay.remove();scene.scene?.resume?.();};
  overlay.querySelector("[data-close]")?.addEventListener("click",close);
  overlay.querySelector("[data-advance]")?.addEventListener("click",()=>{if(i<line.length){i=line.length;window.clearInterval(timer);text.textContent=`“${line}”`;hint.textContent="Tap to close";}else close();});
  scene.scene?.pause?.();
}

function talkToKeeper(scene:any){const n=Math.max(0,Math.min(4,Number(scene.zoneState?.["keysFound"]??0)));scene.zoneState["keeperMet"]=true;if(n===0)scene.objective="Restore the Four Seasons — Seasonal Keys 0/4.";showEvelynDialogue(scene,KEEPER_LINES[n]);}
function readGardenLetter(scene:any){scene.zoneState["gardenLetterRead"]=true;scene.openModal({type:"info",title:"For Maria — In Every Season",body:"Some things are beautiful because they last. Others are beautiful because we choose them again with every season.\n\nEvelyn looks toward the Conservatory. “Now you're ready to see what the garden was protecting.”"});scene.objective="All four seasons are restored — enter the Grand Conservatory.";}
