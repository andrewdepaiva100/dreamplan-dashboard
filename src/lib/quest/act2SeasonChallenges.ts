...
function showCard(scene: any, season: string, complete = false) {
  ...
  const hold = complete ? 2200 : 3600; window.setTimeout(() => { card.style.opacity="0"; card.style.transform="translate(-50%, -8px)"; window.setTimeout(() => card.remove(), 280); }, hold);
}
...
function setupSummer(scene:any) {
  ...
  try { spots.forEach(([tx,ty],i)=>{ const key=scene.textures?.exists?.("enemy-bramble-guardian")?"enemy-bramble-guardian":scene.textures?.exists?.("enemy-rose-crawler")?"enemy-rose-crawler":"enemy-rush"; const speed=42+i*3; const e=scene.spawnEnemy(scene.wx(tx),scene.wy(ty),key,0,true); if(!e)return; e.setData?.("seasonMiniGame","Summer");e.setData?.("summerGuardian",true);e.setData?.("summerAwake",false);e.setData?.("summerSpeed",speed);e.setData?.("hp",SUMMER_GUARDIAN_HP);e.setData?.("maxhp",SUMMER_GUARDIAN_HP);e.setVelocity?.(0,0);e.setTint?.(0xffcf62);scene.__summerGuardians.push(e); }); }
...
function wakeSummerGuardians(scene:any) {
  ...
  for(const e of scene.__summerGuardians??[]){if(!e?.active)continue;e.setData?.("summerAwake",true);e.setData?.("speed",Number(e.getData?.("summerSpeed")??45));}
}
...
function hitSummerGuardian(scene:any,enemy:any){if(!enemy?.getData?.("summerAwake")){enemy.setVelocity?.(0,0);return false;}const damage=Math.max(1,Number(scene.equippedWeapon?.()?.damage??1));const hp=Math.max(0,Number(enemy.getData?.("hp")??SUMMER_GUARDIAN_HP)-damage);enemy.setData?.("hp",hp);scene.floatText?.(enemy.x,enemy.y,`-${damage}`,"#ffe9a8");enemy.setTint?.(0xffe39a);scene.time?.delayedCall?.(90,()=>enemy?.active&&enemy.setTint?.(0xffcf62));return hp<=0;}
