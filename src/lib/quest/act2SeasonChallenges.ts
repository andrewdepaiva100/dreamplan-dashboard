...
const INTRO_COPY: Record<string, { kicker: string; body: string }> = {
  Spring: { kicker: "RENEWAL", body: "The garden remembers its bloom. Wake the flower beds in the order they answer you." },
  Summer: { kicker: "PERSEVERANCE", body: "The garden has grown wild. Clear the bramble guardians protecting the Summer Key." },
...
function ensureState(scene: any) {
  ...
  if (!existing || typeof existing !== "object") {
    scene.zoneState["seasonMiniGames"] = {
      ...
      Autumn: { progress: 0, done: false }, Winter: { lamps: [false,false,false], done: false },
    };
  }
  return scene.zoneState["seasonMiniGames"];
}
function seasonState(scene: any, season: string) { return ensureState(scene)[season]; }
function challengeDone(scene: any, season: string) { return seasonState(scene, season)?.done === true; }
function seasonKey(scene: any, season: string) { return (scene.interactables ?? []).find((it: any) => it?.enabled && it.kind === "season-key" && it.id === season); }
...
function addChallengeInteractable(scene: any, season: string, tx: number, ty: number, texture: string, label: string, id: string, index: number) {
  ...
  const it = scene.addInteractable(scene.wx(tx), scene.wy(ty), texture, "season-mini-game", label, { id: `${season}:${id}`, radius: 72 });
  if (!it) return null;
  it.data = { season, challengeId: id, index }; it.obj.setTint?.(TINTS[season]).setScale?.(.84); return it;
}
function setKeyLockedVisual(scene: any, season: string) {
  const key = seasonKey(scene, season); if (!key) return;
  if (challengeDone(scene, season)) { key.obj.clearTint?.(); key.obj.setAlpha?.(1); key.label = `Open ${season} Key`; }
  else { key.obj.setTint?.(0x718096).setAlpha?.(.58); key.label = `Complete the ${season} trial first`; }
}
function showCard(scene: any, season: string, complete = false) {
  if (typeof document === "undefined") return;
  document.getElementById("quest-season-card")?.remove(); const copy = INTRO_COPY[season]; if (!copy) return;
  const card = document.createElement("div"); card.id = "quest-season-card";
  Object.assign(card.style, { position:"fixed", left:"50%", top:"11%", transform:"translate(-50%, -12px)", width:"min(560px, calc(100vw - 30px))", zIndex:"10010", pointerEvents:"none", opacity:"0", transition:"opacity 260ms ease, transform 260ms ease", borderRadius:"18px", border:`1px solid #${TINTS[season].toString(16).padStart(6,"0")}`, background:"linear-gradient(145deg, rgba(9,20,40,.97), rgba(24,23,48,.96))", boxShadow:"0 18px 45px rgba(0,0,0,.42), inset 0 0 26px rgba(255,255,255,.025)", color:"#fff", padding:"14px 18px 15px", textAlign:"center", fontFamily:"Georgia, serif" });
  ...
  card.innerHTML = `<div style="font-size:10px;letter-spacing:.22em;font-weight:900;color:#${TINTS[season].toString(16).padStart(6,"0")};text-transform:uppercase">Wedding Garden</div><div style="font-size:22px;font-weight:800;margin-top:4px;color:#fff7df">${title}</div><div style="font-family:system-ui,sans-serif;font-size:13px;line-height:1.45;color:rgba(255,255,255,.78);margin-top:6px">${body}</div>`;
  document.body.appendChild(card); requestAnimationFrame(() => { card.style.opacity="1"; card.style.transform="translate(-50%, 0)"; });
  const hold = complete ? 2200 : 6000; window.setTimeout(() => { card.style.opacity="0"; card.style.transform="translate(-50%, -8px)"; window.setTimeout(() => card.remove(), 280); }, hold);
}
function finishTrial(scene: any, season: string) {
  const st = seasonState(scene, season); if (st.done) return; st.done = true; setKeyLockedVisual(scene, season);
  ...
  scene.emitToast?.(`${season} answers. Its Seasonal Key is ready.`); scene.pushHud?.(true); showCard(scene, season, true);
}
...
function setupWinter(scene:any){const spots=[[88,77],[97,72],[105,77]];const st=seasonState(scene,"Winter");if(!st.done)st.lamps=[false,false,false];scene.__winterTrialNodes=spots.map(([x,y],i)=>addChallengeInteractable(scene,"Winter",x,y,"lamp","Lighten the lamp",`lamp-${i}`,i));refreshWinterVisual(scene);}
function refreshWinterVisual(scene:any){const st=seasonState(scene,"Winter");(scene.__winterTrialNodes??[]).forEach((it:any,i:number)=>{if(!it?.obj?.active)return;const lit=Boolean(st.lamps?.[i]);it.obj.setTint?.(lit?0xffe5a3:0x7f91ad).setAlpha?.(lit?1:.48);it.label=lit?"This lamp is glowing":"Lighten the lamp";});}
function winterInteract(scene:any,it:any){const st=seasonState(scene,"Winter"),i=Number(it.data?.index??0),lamps=[...(st.lamps??[false,false,false])];if(lamps[i]){scene.emitToast?.("This Winter lamp is already glowing.");return;}lamps[i]=true;st.lamps=lamps;scene.spawnSparkle?.(it.obj.x,it.obj.y,TINTS.Winter,8);refreshWinterVisual(scene);if(lamps.every(Boolean)){for(const node of scene.__winterTrialNodes??[])if(node?.enabled)scene.removeInteractable?.(node);[[88,77],[97,72],[105,77]].forEach(([x,y])=>decor(scene,"lamp",x,y,0xffe5a3,.84,1));finishTrial(scene,"Winter");}else{const n=lamps.filter(Boolean).length;scene.objective=`Winter — lighten all three lamps (${n}/3 glowing).`;scene.emitToast?.(`${n}/3 Winter lamps are glowing.`);scene.pushHud?.(true);}}
function setupMiniGames(scene:any){ensureState(scene);setupSpring(scene);scene.__summerTrialInitialized=false;scene.__summerGuardiansAwake=false;setupAutumn(scene);setupWinter(scene);SEASONS.forEach(s=>setKeyLockedVisual(scene,s));}
